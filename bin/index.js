...
function createCalendar(initialDate = new Date(), outStream = stdout) {
    ...
    function formatNumber(num, length, sign = ' ') {
    ...
    function getConent() {
        ...
        function colorDay(d) {
            ...
            if (renderDate.getFullYear() === new Date().getFullYear() &&
                renderDate.getMonth() === new Date().getMonth() &&
                new Date().getDate() === d) {
                return kleur.green().bold(renderDay);
            }
            else if (renderDate.getFullYear() === year &&
        ...
        let content = Array(firstWeekday).fill('    ').join('') +
            Array(totalDays)
                .fill(1)
                .map((_, i) => ` ${colorDay(i + 1)} ` + ((firstWeekday + i) % 7 === 6 ? '\n' : ''))
                .join('');
        if (!content.endsWith('\n'))
            content += '\n';
        return content;
    ...
    function render() {
        if (!isFirstRender) {
            ...
            if (lines > 0)
                outStream.write(escapes.eraseLines(lines));
        }
        else {
            isFirstRender = false;
        }
...
function ActionListen(action, inputStream = stdin, outStream = stdout) {
    ...
    const keypress = (str, key) => {
        if (keyMapAction.has(key.name)) {
            action.action(keyMapAction.get(key.name));
        }
        if (key.name === 'escape')
            return exit();
    };
